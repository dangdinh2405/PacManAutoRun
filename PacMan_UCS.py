...
class PacmanGame:
    ...
    def draw_misc(self):
        ...
        BFS_text = self.font.render('[UCS]', True, 'green')
    ...
    def Simulator(self):
        for i in range(len(self.point[0])):
            pygame.draw.circle(self.screen, 'red', (self.point[0][i][1] * self.num2 + (0.5 * self.num2), self.point[0][i][0] * self.num1 + (0.5 * self.num1)), 4)
            pygame.display.flip()
        ...
        if self.path:
            if len(self.path) == 1:
                ...
                if dx1 - dx == -1 and dy1 - dy == 0:
                    ...
                    new_y = player_y + 1 * self.player_speed
                    self.direction = 3
                if new_x == self.ucs.cd_array[dx1][dy1][0] and new_y == self.ucs.cd_array[dx1][dy1][1]:
                    x, y = dx1, dy1
                    self.path.pop(0)
